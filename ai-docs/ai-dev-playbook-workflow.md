...
The AI Dev Playbook supports three categories of template variables:

1. **Core Development Settings**: Basic variables for coding style, testing framework, etc.
2. **Agent Behavior Settings**: Control how agents respond and what they prioritize
3. **Customization Boundaries**: Define limits for what agents can and cannot do

#### Agent Behavior Settings

- `VERBOSITY_LEVEL`: Controls the level of detail in agent responses
  - `minimal`: Just the code or artifacts with minimal explanation
  - `balanced`: Code with key explanations
  - `detailed`: Comprehensive explanations of all decisions

- `CODE_GENERATION_STYLE`: Controls how much code the agent generates
  - `minimal`: Only essential code, no extras
  - `conservative`: Core functionality with basic error handling
  - `comprehensive`: Full implementation with extensive error handling and edge cases

- `EXPLANATION_DETAIL`: Controls the depth of explanations for design decisions
  - `low`: Basic rationale
  - `medium`: Key decisions explained
  - `high`: Detailed explanations including alternatives considered

#### Using Behavior Settings in Prompts

When invoking an agent, you can override behavior settings:

```
Using @workspace .ai-dev/prompts/03-coder-agent.md with variables from @workspace .ai-dev/config/variables.json and VERBOSITY_LEVEL=minimal, implement...
```
...
1. **Playbook Core Principles**
   - Defined in `ai-docs/ai-dev-playbook-spec.md`
...
For more details on instruction hierarchy, see [instruction-hierarchy.md](instruction-hierarchy.md).

### Specialized Agent Variants

The AI Dev Playbook now includes specialized variants of agents optimized for different use cases:

#### Interactive Development
Use interactive variants when you need detailed explanations and guidance:
```
Using @workspace .ai-dev/prompts/variants/03a-interactive-coder-agent.md, implement...
```

These variants provide comprehensive explanations, alternative approaches, and educational context.

#### Automated Workflows
Use automated variants for CI/CD pipelines or batch processing:
```
Using @workspace .ai-dev/prompts/variants/03b-automated-coder-agent.md, implement...
```

These variants generate minimal, production-ready code with essential comments only.

#### Code Review
Use review variants when evaluating existing code:
```
Using @workspace .ai-dev/prompts/variants/03c-review-coder-agent.md, review...
```

These variants focus on constructive feedback and specific improvement suggestions.

For guidance on selecting the right agent for each task, see [agent-selection-guide.md](agent-selection-guide.md).